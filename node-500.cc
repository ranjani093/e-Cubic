...
main (int argc, char *argv[])
{
 ...
 // bool tracing = true;
  ...
  std::string traceFile = "scratch/node-500.ns_movements";
  uint32_t maxBytes = 0;
...
//  AnimationInterface anim("node-500.xml");
...
// Enabling the XML Flow Monitor
 if (enableFlowMonitor)
    {
      flowmonHelper.SerializeToXmlFile ("node-500.flowmon", false, false);
    }
  Simulator::Destroy ();
  return 0;
}
